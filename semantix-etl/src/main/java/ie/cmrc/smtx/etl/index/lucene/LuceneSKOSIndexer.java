 ...
 * all SKOS resources of a thesaurus
 * @author Yassine Lassoued
 * @deprecated Not to be used for the time being.
 */
public final class LuceneSKOSIndexer implements SKOSIndexer {
    ...
    /**
     * List of languages for which the indexer will index the resource labels and definitions 
     */
    ...
    static {
        ...
        annotationFields.put(SKOSAnnotationProperty.altLabel, new IndexField.Searchable[]{IndexField.Searchable.LABEL, IndexField.Searchable.IX_LABEL});
        annotationFields.put(SKOSAnnotationProperty.hiddenLabel, new IndexField.Searchable[]{IndexField.Searchable.LABEL, IndexField.Searchable.IX_LABEL});
        ...
        annotationStoring.put(SKOSAnnotationProperty.altLabel, new Field.Store[]{Field.Store.YES, Field.Store.NO});
        annotationStoring.put(SKOSAnnotationProperty.hiddenLabel, new Field.Store[]{Field.Store.NO, Field.Store.NO});
     ...
     */
    public LuceneSKOSIndexer(List<String> languageCodes) {
     ...
     */
    public LuceneSKOSIndexer(List<String> languageCodes, boolean verbose) {
        this.setLanguages(languageCodes);
     ...
     */
    public LuceneSKOSIndexer(List<String> languageCodes, boolean verbose, boolean indexTransitiveCollections) {
        this.verbose = verbose;
        this.setLanguages(languageCodes);
    ...
    public boolean indexSKOSThesaurus(SKOS thesaurus, File indexDirFile) throws NotDirectoryException, IOException {
        ...
        if (thesaurus != null) {
            if (indexDirFile!=null) {
                ...
                if (verbose) System.out.println("  - Indexing resources. Please be patient as this may take a few minutes...");
                int counter = 0;
                int errors = 0;
                ...
                while (iter.hasNext()) {
                    ...
                    if (doc != null) {
                        try {
                            ...
                            errors++;
                            if (verbose) System.out.println("    * ERROR: Could not index resource \""+resource.getURI()+"\"!");
                        }
                    }
                }
                iter.close();
                if (verbose) System.out.println("      --> Indexed "+counter+" resources(s) with "+errors+" error(s).");
     ...
     * {@link ie.cmrc.skos.SKOSResource} and stores them in an index document
     * {@code org.apache.lucene.document.Document} ready to be added to an
     * index writer
     * @param resource Concept to index
     * @return {@code org.apache.lucene.document.Document} containing the
     * relevant fields and values to index.
     */
    private Document indexResource(SKOSResource resource) {
        Document doc = new Document();
        
        SKOSType type = resource.getSkosType();
        doc.add(new StringField(IndexField.Filterable.TYPE.fieldName(), type.name(), Field.Store.YES));
        ...
        List<SKOSConceptScheme> css = resource.getConceptSchemes();
        for (SKOSConceptScheme cs: css) {
            doc.add(new StringField(IndexField.Filterable.CS.fieldName(), cs.getURI(), Field.Store.YES));
        }
        
        if (SKOSType.Concept.equals(type) || SKOSType.Collection.equals(type)) {
            SKOSCollectionMember member = (SKOSCollectionMember)resource;
        ...
        for (SKOSAnnotationProperty annotationProp: annotationProps) {
            List<Term> annotations = resource.getAnnotations(annotationProp);
